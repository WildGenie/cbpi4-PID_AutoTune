    ...
    async def run(self):
        ...
        if setpoint == 0:
            if fixedtarget < current_value:
                self.cbpi.notify(
                    'PID AutoTune',
                    f'You have not defined a target temp and current temp is above fallback value of {fixedtarget} °{self.TEMP_UNIT}. Please choose target temp above current temp or wait until systme has cooled down.',
            ...
            self.cbpi.notify(
                'PID AutoTune',
                f'You have not defined a target temp. System will set target to {fixedtarget} °{self.TEMP_UNIT} and start AutoTune',
        ...
        try:
            ...
            self.cbpi.notify(
                'PID autoTune', f'AutoTune Error: {str(e)}', NotificationType.ERROR
            )
        ...
        try:
            ...
            logging.error(f"PIDAutoTune Error {e}")
            await self.actor_off(self.heater)
            await self.stop()
        finally:
            await self.actor_off(self.heater)
...
class AutoTuner(object):
	...
	def __init__(self, setpoint, outputstep=10, sampleTimeSec=5, lookbackSec=60,
	    ...
	    self._getTimeMs = self._currentTimeMs if getTimeMs is None else getTimeMs
	...
	def run(self, inputValue):
	    ...
	    if self._state in [
	        AutoTuner.STATE_OFF,
	        AutoTuner.STATE_SUCCEEDED,
	        AutoTuner.STATE_FAILED,
	    ]:
	    ...
	    if inflection and (self._peakCount > 4):
	        ...
	        for i in range(len(self._peaks) - 2):
